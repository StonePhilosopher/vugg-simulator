...
| **O4a** ✅ 2026-07-07 (`5260134`, render-only) · O4b pending | engulfment made VISIBLE. The Sweetwater enclosure mechanic (`_check_enclosure`) has tagged `enclosed_by`/`enclosed_crystals` sim-side all along + the 2D topo map dots them (js/99b) — the 3D view never had a face. Now a guest renders as a small OPAQUE grain INSIDE its host (js/99i two-pass: `isInclusion` → clarity 0 matte grain keeping its own mineral-class colour = poikilotopic; seated off the host's **RENDERED** bounds via pure `_o4InclusionLocalPos`, NOT the sim's uncapped c_length — Naica selenite is 66 mm sim vs ~13 mm rendered, the bug the test pins), revealed by Depth-A host translucency; an OPAQUE host rightly HIDES its inclusions (honest). Skips O2 clip + cluster satellites. Kernel-truth: naica 35/35 opaque+inside, 569-px with/without framebuffer diff; reactivated-fluorite galena/rhodochrosite hosts hide theirs. **O4b** (the SIM-bump half, LATER): replace the position-STRING adjacency in `_check_enclosure` (js/85c:696) with real ring/cell proximity; `coats_front` embedded-vs-front-coating classes (front-coating → O5). Instruments: `tools/o4-enclosure-census.mjs`, `tests-js/o4-engulfment.test.ts`. | O4a render-only 0/38 · O4b SIM bump |
| **O5 — THE ORIGINATING ASK** (boss 2026-07-08: *"definitely not optional polish… but a task for another time"*) | inclusion-perturbed regrowth — the DIRECT answer to the founding phrasing *"how uneven mineral inclusions can alter later layers of growth."* NOT the embedded grain (that's O4a, inert poikilotopic — correctly unperturbing) but a **film on the growth front**: per-face-class inhibitor coverage φ from a hiatus dusting → masked faces → **phantom horizon** (js/27 already tags `is_phantom`/`phantom_surfaces` from dissolution — O5 EARNS it from masking + renders it) → renewed growth; masked prism + free termination → the **sceptre** EARNED by ELO (the alpine arc's DECLARED-but-painted Grimsel sceptres get their real mechanism — a standing debt); σ×impurity above threshold → **split growth** up the sheaf→bowtie→spherulite ladder (saddle dolomite is the shipped low-grade rung). Science locked: Takahashi/Sunagawa 2004 (ELO ✓ exact), Shtukenberg 2012 (grades ✓). O4b's `coats_front` boolean is the fork that routes front-coating inclusions here. **Sequencing is the only variable, never whether — this rung is load-bearing, not quietly reclassified.** | SIM bump |
