...
> shigar aqua size (probe KILLED the etch hypothesis — 2-3% loss, pure sculpture; the cap is
> KINETICS, 23.8 µm/step at σ 3.91 vs albite's 1661; levers = scenario Be budget → global
> beryl growth_rate_mult 0.25, census-gated; acceptance = star ≥20 mm AND the ≤0.5 mm fry
> preserved — the boss's New Hampshire principle: model the DISTRIBUTION's right tail, not a
> floor) · Door 2 UV scale audit (uv-zone-census tool first → 98c recalibration render-only →
