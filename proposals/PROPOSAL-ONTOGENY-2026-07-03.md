...
| **O4a ✅ 2026-07-07 (`5260134`) · O4b ✅ 2026-07-08 (SIM 221, `08287eb` — census `6967c57` found the string gate 81% phantom; geometric adjacency = painted-footprint reach on wall anchors; coats_front shipped, 14 front / 74 embedded — O5's prepared data)** | **O4a (render-only, shipped):** renderer reads `enclosed_crystals`; guest renders as a small OPAQUE grain INSIDE its host, host translucency (Depth-A) reveals it; an opaque host rightly HIDES it (honest). Two-pass placement off the host's **rendered** bounds (`_o4InclusionLocalPos`) — the sim's uncapped c_length overshot (Naica 66 mm sim vs ~13 mm rendered) and spilled guests outside. Kernel-truth: naica 35/35 opaque+inside, 569-px with/without diff. **O4b (SIM bump, later):** `coats_front` split — embedded-inert (sand calcite, host stays one crystal) vs front-coating (routes to O5); upgrade enclosure adjacency from the string-compare (js/85c:696) to the ring/cell proximity query. | poikilotopic enclosure (✓) | O4a render 0/38 · O4b bump | M |
