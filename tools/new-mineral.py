...
def insert_json_entry(args, entry):
    """Append the entry to data/minerals.json (single source of truth).

    Pre-flatten this also wrote a docs/data/minerals.json mirror because
    GitHub Pages served from /docs. Pages now serves from repo root and
    fetches data/minerals.json directly — no mirror needed.
    """
...
**JSON entry**: ✅ Already inserted into `data/minerals.json` (the single
spec source — fetched at runtime by index.html, required by agent-api,
read by vugg.py). Open and customize the TODO comments (description,
audit_status, habit_variants, etc.).
...
Find: `const MINERAL_SPEC_FALLBACK = {{` in index.html. Add this
one-liner inside:
...
index.html. Add this entry to BOTH (or use sed):
...
def main():
    ...
    spec_path = insert_json_entry(args, entry)
