...
export function assertIndexMatchesDist(root) {
  ...
  const body = concatDistModules(root).replace(/\r?\n/g, newline);
...
export function runtimeDependencyFiles(root) {
  ...
  const narratives = walkFiles(path.join(root, 'narratives'), name => name.endsWith('.md'));
  const files = [...distModuleFiles(root), ...explicit, ...narratives];
  for (const file of files) {
