...
// The exact-executable owned-browser receipt drives the public controls through
// complete Creative and Simulation lessons, including pause, Skip, final
// teardown, and the geological-save/tutorial-overlay policy.
// v282 (August 2026): player-owned specimen names cross one shared text-only
// presentation boundary in Library and Record Groove. Strip uploads are
// content-addressed in a separately labeled/import-evicted namespace, and
// their full serialized payload is rehashed on read. Recent-cache eviction
// and insertion publish atomically. The owned-browser receipt follows the
// collected specimen through Record Groove, downloads/reimports/plays its
// strip, traverses topology/Helicoid controls, and checks all four surfaces on
// a phone viewport.
...
// machine-readable envelopes rather than claiming whole-response parity. The
// public async command consumes an immutable constructor receipt and rechecks
// its launch generation before metadata or stepping; an unknown, cancelled,
// or superseded request can never relabel the ambient prior simulation. Every
// visible constructor owns a frozen run identity and commissions the current
// specimen source, so a headless label or older mode handle cannot leak into a
// later dump.
...
// v285 (August 2026): CROSS-01 closes three UI/science identity crossings.
// Formation counterfactuals preserve the executable FluidChemistry prototype,
// so advice calls the same supersaturation surface as production. Collection,
// Record Groove, save receipts, and finish staging require positive signed,
// non-phantom remaining layer inventory; a historical numeric layer count is
// portable census testimony, not proof that solid survives. Timeline fields
// are canonicalized before commissioning, disclose their physical domain, and
// enforce that domain pointwise after bulk-to-pore-fluid propagation. Generic
// sulfur timelines are refused because they cannot provide valence-specific
// boundary and ledger authority.
const SIM_VERSION = 285;
...
const MODEL_DIGEST = [
  ...
  'diagnosis:production-nucleator+prototype-preserving-fluid-counterfactual+local-max-context+causal-supersat+calibrated-budget-v6',
  ...
  'tsumeb:TSNB159-confirmed-gypsum-recharge+exact-authored-acid-stage-sulfate+Cu-only-enrichment+Ca-sulfate-recharge+generic-fluid-receipts+secondary-sulfides-aspirational-until-parent-replacement+TSNB301-questionable-rhodochrosite-excluded-v3',
  ...
  'agent-runtime:explicit-sulfur-valence+ambiguous-S-rejected+owner-bound-MVT-90sulfide-30sulfate+narrow-physics-parity+explicit-response-scope+shared-crystal-serialization-core+browser-shape-seed-applied-and-replay-bound+exact-constructor-receipt+post-await-launch-generation+sim-owned-metadata+last-commissioned-visible-selection-v4',
  ...
  'collection-product:typed-bounded-backup-records+released-safe-integer-zone-count-migration+census-only-numeric-zones+signed-nonphantom-remaining-solid-required-across-collect-groove-save-finish+current-zone-array-cap10000+atomic-oversize-collect-refusal+actual-zone-array-required-for-groove+exact-player-owned-name+shared-text-only-library-and-record-groove-fields-and-actions-v4',
