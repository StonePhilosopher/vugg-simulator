...
function grow_goethite(crystal, conditions, step) {
  ...
  if (sigma < 1.0) {
    ...
    if (crystal.total_growth_um > 3 && conditions.fluid.pH < 3.0) {
      ...
      // Phase 1e: Fe credit handled by applyStoichiometricGrowthBudget via MINERAL_DISSOLUTION_RATES.goethite.
...
function grow_lepidocrocite(crystal, conditions, step) {
  ...
  if (sigma < 1.0) {
    if (crystal.total_growth_um > 5 && conditions.fluid.pH < 3.0) {
      ...
      // Phase 1e: Fe credit handled by applyStoichiometricGrowthBudget via MINERAL_DISSOLUTION_RATES.lepidocrocite.
