...
// event intervals. Caps lifted where growth budget creates more
...
function event_jeffrey_mine_serpentinization_onset(c) {
  ...
  c.fluid.S = Math.max(0, c.fluid.S - 0.5);     // serpentinization consumes S to magnetite (NOT growth-budget double-debit; sulfide minerals dispatch separately)
...
function event_jeffrey_mine_dike_alteration(c) {
  ...
  // line REMOVED (growth budget handles it via diopside/vesuvianite/
  ...
  // Mg consumption REMOVED — growth budget via stoichiometry now does it
...
function event_jeffrey_mine_mid_rodingite(c) {
  ...
  // consumption by vesuvianite. With growth budget, vesuvianite debits
...
function event_jeffrey_mine_late_ca_silicates(c) {
  ...
  // members.
  //
  // v123 tune: Mg "consumption" REMOVED (growth budget handles it). Ca
  ...
  // Mg consumption REMOVED (growth budget via tremolite/actinolite/diopside)
